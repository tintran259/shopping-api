...
export class ShippingAddressDto {
  ...
  @ApiProperty({ description: 'Province code' }) @Type(() => Number) @IsInt() provinceCode: number;
  @ApiProperty({ description: 'Ward code' }) @Type(() => Number) @IsInt() wardCode: number;
...
export class CheckoutDto {
  ...
  @ApiPropertyOptional({ format: 'uuid', description: 'Use a saved address (delivery)' })
  ...
  @IsOptional() @IsNumberString() shippingFee?: string;

  @ApiPropertyOptional({ type: InvoiceDto, description: 'VAT invoice (B2B)' })
  @IsOptional() @ValidateNested() @Type(() => InvoiceDto)
  invoice?: InvoiceDto;
